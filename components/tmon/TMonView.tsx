...
import { computeTickerPnl } from "@/lib/services/tmonService"
...
// 20 distinct colours for tickers
const TICKER_COLORS = [
  "#60a5fa", "#34d399", "#f87171", "#fbbf24", "#a78bfa",
  "#f472b6", "#2dd4bf", "#fb923c", "#a3e635", "#e879f9",
  "#22d3ee", "#facc15", "#4ade80", "#f43f5e", "#818cf8",
  "#fb7185", "#38bdf8", "#86efac", "#fcd34d", "#c084fc",
]
...
type TopNValue = typeof TOP_N_OPTIONS[number]["value"]
...
function TMonTooltip({ active, payload, label }: {
  active?: boolean
...
export function TMonView() {
  ...
  const [topN, setTopN] = useState<TopNValue>(10)
  ...
  const { points: chartData, tickers } = useMemo(
    () => computeTickerPnl(filteredTrades, topN),
    [filteredTrades, topN]
  )
  ...
  return (
    <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <h1 className="text-xl font-semibold tracking-tight">Ticker PnL Monitor</h1>
      ...
      <div className="flex items-center gap-3">
        <span className="flex items-center gap-1 text-sm text-muted-foreground">
          ...
          <InfoTooltip
            content={
              <>
                Tickers are ranked by <strong>absolute cumulative PnL</strong> — the biggest movers (both profitable and unprofitable) appear first.
                <br /><br />
                Use <strong>All</strong> only if you trade a small number of symbols — too many lines become unreadable.
              </>
            }
          />
        </span>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Cumulative PnL by Ticker</CardTitle>
          <p className="text-xs text-muted-foreground mt-0.5">
            Each line shows the running total PnL for a ticker over time.
            Ranked by absolute final PnL — showing the {isFinite(topN) ? `top ${topN}` : "all"} most impactful symbol{topN !== 1 ? "s" : ""}.
          </p>
        </CardHeader>
        <CardContent>
      ...
      {chartData.length > 0 && tickers.length > 0 && (() => {
        ...
        return (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {sorted.map((ticker) => {
              ...
              const final = (last[ticker] as number) ?? 0
              const positive = final >= 0
              return (
                <Card key={ticker} className="text-center">
                  <CardContent className="pt-3 pb-2 px-3">
                    <div className="mb-1 h-2 w-2 rounded-full mx-auto" style={{ backgroundColor: TICKER_COLORS[i % TICKER_COLORS.length] }} />
                    <p className="text-xs font-medium text-muted-foreground truncate">{ticker}</p>

...
export function HomeView() {
  ...
  const runFetch = useCallback((force: boolean) => {
    ...
    // No API keys configured (e.g. admin impersonating another user) —
    // load all cached trades for this telegramId directly from the DB.
    if (configs.length === 0) {
      setTrades([])
      setLoadedExchanges([])
      setExchangeErrors({})
      ...
      fetch('/api/trades-cache/all', {
        ...
        .then((data) => {
          ...
          t.sort((a, b) => new Date(b.closeTime).getTime() - new Date(a.closeTime).getTime())
          setTrades(t)
        })
        .catch(() => {})
